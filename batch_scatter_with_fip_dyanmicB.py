# scatter plot of intensity ratio vs loop lenght (Dyanmic Gauss
import os
import glob
import numpy as np
...
do_dynamic_split = True
b90_skip_threshold = 100.0   # if 90th percentile of closed B is below this, don't split
min_per_side = 300
alpha = 0.01
grid_points = 60
...
def auto_split_gauss(B_vals, y_vals, min_per_side=300, alpha=0.01, grid_points=60):
    x = np.log10(B_vals)

    n = len(x)
    if n < (2 * min_per_side + 5):
        return None, {"reason": "not enough points for split search"}

    # Single fit
    m0, c0, _, _, _ = linregress(x, y_vals)
    resid0 = y_vals - (m0 * x + c0)
    sse0 = np.sum(resid0**2)

    lo = np.percentile(B_vals, 10)
    hi = np.percentile(B_vals, 90)
    if lo <= 0 or hi <= 0 or lo >= hi:
        return None, {"reason": "bad percentile range"}

    candidates = np.logspace(np.log10(lo), np.log10(hi), grid_points)
    ...
    for B0 in candidates:
        ...
        n1 = np.sum(mask1)
        n2 = np.sum(mask2)
        if n1 < min_per_side or n2 < min_per_side:
            continue

        x1, y1 = x[mask1], y_vals[mask1]
        x2, y2 = x[mask2], y_vals[mask2]

        m1, c1, _, _, _ = linregress(x1, y1)
        m2, c2, _, _, _ = linregress(x2, y2)

        resid1 = y1 - (m1 * x1 + c1)
        resid2 = y2 - (m2 * x2 + c2)
        sse2 = np.sum(resid1**2) + np.sum(resid2**2)

        # AICc comparison (k0=2 params, k2=4 params)
        k0 = 2
        k2 = 4
        ...
        aic0 = n * np.log(sse0 / n) + 2 * k0
        aic2 = n * np.log(sse2 / n) + 2 * k2

        # small-sample correction
        aicc0 = aic0 + (2 * k0 * (k0 + 1)) / max(n - k0 - 1, 1)
        aicc2 = aic2 + (2 * k2 * (k2 + 1)) / max(n - k2 - 1, 1)

        delta_aicc = aicc0 - aicc2  # positive means 2-line better

        # F-test style significance (nested models)
        df1 = k2 - k0
        df2 = n - k2
        if df2 <= 0:
            continue

        F = ((sse0 - sse2) / df1) / (sse2 / df2)
        p = f.sf(F, df1, df2)

        if best is None or aicc2 < best["aicc2"]:
            best = {
                "B0": float(B0),
                "m0": float(m0), "c0": float(c0), "sse0": float(sse0), "aicc0": float(aicc0),
                "m1": float(m1), "c1": float(c1),
                "m2": float(m2), "c2": float(c2),
                "sse2": float(sse2), "aicc2": float(aicc2),
    ...
    if best is None:
        return None, {"reason": "no candidate passed min_per_side"}

    # Accept split only if:
    # 1) statistically significant improvement
    # 2) meaningful AICc improvement (stops false fires)
    if best["p"] >= alpha or best["delta_aicc"] < 10:
        return None, {"reason": "split not significant", **best}
...
for ar_id in ar_list:
    ...
    # plt.tight_layout()
    outname = os.path.join(output_dir, f"AR{ar_id}_Abundance_length_with_fip_dynamicG.png")
    ...
    for idx, element in enumerate(order):
        ...
        # dynamic split (per ar, per element) with weak-ar gate
        split_gauss = None
        
        # decide split using closed pixels only
        bc = B_vals[closed_mask_flat]
        yc = abund_vals[closed_mask_flat]
        
        b90 = np.percentile(bc, 90) if bc.size > 0 else np.nan
        
        if (not do_dynamic_split) or (not np.isfinite(b90)) or (b90 < b90_skip_threshold):
            # no split: treat everything as one regime
            reason = f"no split (b90={b90:.2f}g < {b90_skip_threshold:.0f}g)" if np.isfinite(b90) else "no split (no closed data)"
        else:
            split_gauss, info = auto_split_gauss(
                bc, yc,
                min_per_side=min_per_side,
                alpha=alpha,
                grid_points=grid_points
            )
            reason = info.get("reason", "accepted") if split_gauss is None else "accepted"
        
        # masks for plotting on full arrays
        if split_gauss is None:
            mask_low = np.ones_like(B_vals, dtype=bool)
            mask_high = np.zeros_like(B_vals, dtype=bool)
        else:
            mask_low = (B_vals < split_gauss)
            mask_high = ~mask_low
        
        # only count closed pixels for fit feasibility
        n_low = np.sum(mask_low & closed_mask_flat)
        n_high = np.sum(mask_high & closed_mask_flat)
        
        have_low = n_low >= 2
        have_high = n_high >= 2
        
        print(f"[{element}] split={split_gauss} ({reason}) closed: low={n_low}, high={n_high}")
        ...
        # low side (or whole range if no split)
        if have_low:
            b_low = B_vals[mask_low & closed_mask_flat]
            y_low = abund_vals[mask_low & closed_mask_flat]
        
            slope_lin_low, intercept_lin_low, *_ = linregress(b_low, y_low)
        
            log_b_low = np.log10(b_low)
            slope_log_low, intercept_log_low, *_ = linregress(log_b_low, y_low)
        
            hi_edge = split_gauss if split_gauss is not None else b_low.max()
            x_fit_log10_low = np.logspace(np.log10(b_low.min()), np.log10(hi_edge), 100)
            y_fit_log_low = slope_log_low * np.log10(x_fit_log10_low) + intercept_log_low
        
        # high side only if split exists
        if have_high and (split_gauss is not None):
            b_high = B_vals[mask_high & closed_mask_flat]
            y_high = abund_vals[mask_high & closed_mask_flat]
        
            slope_lin_high, intercept_lin_high, *_ = linregress(b_high, y_high)
        
            log_b_high = np.log10(b_high)
            slope_log_high, intercept_log_high, *_ = linregress(log_b_high, y_high)
        
            x_fit_log10_high = np.logspace(np.log10(split_gauss), np.log10(b_high.max()), 100)
            y_fit_log_high = slope_log_high * np.log10(x_fit_log10_high) + intercept_log_high
        ...
        if have_low:
            if split_gauss is None:
                logfit_label_low = f'fit (no split): y={slope_log_low:.2e}·log_10(x)+{intercept_log_low:.2f}'
            else:
                logfit_label_low = f'fit < {split_gauss:.0f}g: y={slope_log_low:.2e}·log_10(x)+{intercept_log_low:.2f}'
        ...
        if have_high and (split_gauss is not None):
            logfit_label_high = f'fit >= {split_gauss:.0f}g: y={slope_log_high:.2e}·log_10(x)+{intercept_log_high:.2f}'
            logfit_line_high = ax.plot(
                x_fit_log10_high, y_fit_log_high,
                color='red', linestyle='-', alpha=1, zorder=4, linewidth=5
            )[0]
    ...
    outname = os.path.join(output_dir, f"AR{ar_id}_Abundance_B_with_fip_dynamicG.png")
    plt.savefig(outname, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved: {outname}")
...
with open(diagnostics_path, "a") as fdiag:
    ...
    # plt.tight_layout()
    outname = os.path.join(output_dir, "ARall_Abundance_length_with_fip_dynamicG.png")
    plt.savefig(outname, dpi=150, bbox_inches="tight")
    plt.close(fig)
...
with open(diagnostics_path, "a") as fdiag:
    ...
    for idx, element in enumerate(order):
        ...
        # dynamic split (all ars, per element) with weak-ar gate
        split_gauss = None
        ...
        # draw fit lines + legend labels (lowercase)
        if have_low:
            if split_gauss is None:
                label_low = f'fit (no split): y={slope_log_low:.2e}·log_10(x)+{intercept_log_low:.2f}'
            else:
                label_low = f'fit < {split_gauss:.0f}g: y={slope_log_low:.2e}·log_10(x)+{intercept_log_low:.2f}'
        
            line_low = ax.plot(
                x_fit_log10_low, y_fit_log_low,
                color='red', linestyle='--', alpha=1, zorder=4, linewidth=5
            )[0]
            fit_lines.append(line_low)
            fit_labels.append(label_low)
        
        if have_high and (split_gauss is not None):
            label_high = f'fit >= {split_gauss:.0f}g: y={slope_log_high:.2e}·log_10(x)+{intercept_log_high:.2f}'
            line_high = ax.plot(
                x_fit_log10_high, y_fit_log_high,
                color='red', linestyle='-', alpha=1, zorder=4, linewidth=5
            )[0]
            fit_lines.append(line_high)
            fit_labels.append(label_high)
    ...
    outname = os.path.join(output_dir, "ARall_Abundance_B_with_fip_dynamicG.png")
    plt.savefig(outname, dpi=150, bbox_inches="tight")
    plt.close(fig)
    # plt.show
    print(f"Saved: {outname}")
