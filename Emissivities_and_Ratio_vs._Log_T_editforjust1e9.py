...
# reference point per pair (logt0 for ne=1e9) — same values you use in map code
pair_ref_logt0 = {
    ("ca_14", 193.87, "ar_14", 194.40): 6.60,
    ("fe_16", 262.98, "s_13", 256.69):  6.40,
    ("si_10", 258.37, "s_10", 264.23):  6.20,
    ("s_11",  188.68, "ar_11", 188.81): 6.25,
}
...
def _interp_log_ratio(logt0, logt_grid, ratio_curve):
    """
    Return R(logt0) given a ratio curve sampled on logt_grid.
    Works in log-space for numerical stability.
    """
    rc = np.clip(np.asarray(ratio_curve, float), 1e-30, None)
    return 10.0 ** np.interp(float(logt0), np.asarray(logt_grid, float), np.log10(rc))
...
# === print exact theory ratios R for ne=1e9 at your chosen logt0 per pair ===
print("\n=== theory ratios R_th_ref at (ne=1e9) ===")
pair_r_out = {}
for low_ion, low_wvl, high_ion, high_wvl in [
    ("ca_14", 193.87, "ar_14", 194.40),
    ("fe_16", 262.98, "s_13", 256.69),
    ("si_10", 258.37, "s_10", 264.23),
    ("s_11",  188.68, "ar_11", 188.81),
]:
    key = (low_ion, low_wvl, high_ion, high_wvl, 1e9)
    if key not in emissivity_data:
        print(f"{low_ion} {low_wvl} / {high_ion} {high_wvl}: missing ne=1e9 data")
        continue
    low_emiss, high_emiss = emissivity_data[key]
    ratio_curve = np.where(high_emiss > 0, low_emiss / np.maximum(high_emiss, 1e-30), np.nan)
    logt0 = pair_ref_logt0[(low_ion, low_wvl, high_ion, high_wvl)]
    # restrict to finite points for interpolation
    ok = np.isfinite(ratio_curve)
    r_val = _interp_log_ratio(logt0, logT[ok], ratio_curve[ok])
    pair_r_out[(low_ion, low_wvl, high_ion, high_wvl)] = float(r_val)
    print(f"{low_ion} {low_wvl:.2f} / {high_ion} {high_wvl:.2f} at logT0={logt0:.2f}: R = {r_val:.4f}")
print("=================================================================\n")
# ===================================================================
...
def plot_emissivity_ratios(emissivity_data, logT):
    ...
    for idx, (low_ion, low_wvl, high_ion, high_wvl) in enumerate(unique_pairs):
        ...
        for ne, color, linestyle in zip([1e9], colors_ratio[:1], ['--']):
